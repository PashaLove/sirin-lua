...
---@field onMonsterDestroy fun(pMonster: CMonster, byDestroyCode: integer, pAttObj: CGameObject)
---@field onMonsterTransport fun(pMonster: CMonster, dwOldSerial: integer)
local sirinMonsterScheduleMgr = {
	m_SpawnRules = {},
	m_Monsteres = {},
	m_bDebugLog = false,
	m_tLoopTime = 0,
	m_strUUID = 'sirin.lua.sirinMonsterScheduleMgr',
	m_pszLogPath = '.\\sirin-log\\guard\\LuaMonsterSchedule.log',
	m_pszSavePath = '..\\SystemSave\\LuaMonsterSchedule.ini',
}

function sirinMonsterScheduleMgr.initHooks()
	SirinLua.HookMgr.addHook("CMonster__Destroy", HOOK_POS.pre_event, sirinMonsterScheduleMgr.m_strUUID, sirinMonsterScheduleMgr.onMonsterDestroy)
	SirinLua.HookMgr.addHook("CMonsterHelper__TransPort", HOOK_POS.after_event, sirinMonsterScheduleMgr.m_strUUID, sirinMonsterScheduleMgr.onMonsterTransport)
end
...
function SirinMonsterSpawnRule:updateSummonTime()
	if self.m_Script.window and self.m_Script.window > 0  then
		...
		if self.m_tNextBirthTime < tNextWindowMin or self.m_tNextBirthTime > tNextWindowMax then
			...
			Sirin.WritePrivateProfileStringA(self.m_strScriptID, "NextBirth", tostring(self.m_tNextBirthTime), sirinMonsterScheduleMgr.m_pszSavePath)

			if sirinMonsterScheduleMgr.m_bDebugLog then
				...
				Sirin.WriteA(sirinMonsterScheduleMgr.m_pszLogPath, emsg, true, true)
			end
		else
			if sirinMonsterScheduleMgr.m_bDebugLog then
...
function SirinMonster:respawn()
	if self:isLive() then
		...
		if not self.m_pCObj then
			...
			sirinMonsterScheduleMgr.m_Monsteres[self.m_dwObjectSerial] = self
...
function SirinMonster:despawn()
	sirinMonsterScheduleMgr.m_Monsteres[self.m_dwObjectSerial] = nil
...
function SirinMonster:die()
	...
	self.m_pCObj = nil
	Sirin.WritePrivateProfileStringA(self.m_strScriptID, "LastDeath", tostring(self.m_Rule.m_tLastDeathTime), sirinMonsterScheduleMgr.m_pszSavePath)
...
function sirinMonsterScheduleMgr.onMonsterTransport(pMonster, dwOldSerial)
	if sirinMonsterScheduleMgr.m_Monsteres[dwOldSerial] then
		sirinMonsterScheduleMgr.m_Monsteres[pMonster.m_dwObjSerial] = sirinMonsterScheduleMgr.m_Monsteres[dwOldSerial]
		sirinMonsterScheduleMgr.m_Monsteres[dwOldSerial] = nil
	end
end
...
function sirinMonsterScheduleMgr.onMonsterDestroy(pMonster, byDestroyCode, pAttObj)
	if byDestroyCode ~= 0 then
		return
	end

	if sirinMonsterScheduleMgr.m_Monsteres[pMonster.m_dwObjSerial] then
		sirinMonsterScheduleMgr.m_Monsteres[pMonster.m_dwObjSerial]:die()
	end
end
...
function sirinMonsterScheduleMgr.loadScripts()
	...
	repeat
		SirinTmp_MonsterSchedule = FileLoader.LoadChunkedTable(".\\sirin-lua\\threads\\main\\ReloadableScripts\\MonsterSchedule")

		if not SirinTmp_MonsterSchedule then
			Sirin.console.LogEx_NoFile(ConsoleForeground.RED, ConsoleBackground.BLACK, "Failed to load 'Monster schedule' scripts!\n")
			Sirin.WriteA(sirinMonsterScheduleMgr.m_pszLogPath, "Failed to load 'Monster schedule' scripts!\n", true, true)
			break
		end

		if not sirinMonsterScheduleMgr.validateScriptData() then
			break
		end

		sirinMonsterScheduleMgr.init()
		bSucc = true
	until true
...
function sirinMonsterScheduleMgr.validateScriptData()
	...
	repeat
		local luaScript = SirinTmp_MonsterSchedule
			...
			repeat
				...
				if monster_data.ruleStartTime then
					if type(monster_data.ruleStartTime) ~= "string" then
						Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'ruleStartTime' is not a string type!\n", strCode))
						break
					end
				...
				if monster_data.ruleEndTime then
					if type(monster_data.ruleEndTime) ~= "string" then
						Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'ruleEndTime' is not a string type!\n", strCode))
						break
					end
				...
				if type(monster_data.monsterCode) ~= "string" then
					Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'monsterCode' is not a string type!\n", strCode))
					break
				end
				...
				if not newSchedule.monster then
					Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'monsterCode'(%s) not exists!\n", strCode, monster_data.monsterCode))
					break
				end

				if type(monster_data.schedule) ~= "table" then
					Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'schedule' is not a table type!\n", strCode))
					break
				end

				if type(monster_data.schedule.scheduleType) ~= "string" then
					Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'schedule.scheduleType' is not a string type!\n", strCode))
					break
				end
				...
				if not bSucc then
					Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'schedule.scheduleType' invalid value!\n", strCode))
					break
				end

				if type(monster_data.schedule.intervals) ~= "table" then
					Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'schedule.intervals' is not a table type!\n", strCode))
					break
				end
				...
				for k,v in ipairs(monster_data.schedule.intervals) do
					...
					if not v.hour and not v.minute and not v.val then
						Sirin.console.LogEx(ConsoleForeground.YELLOW, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'schedule.intervals[%s]' does not contain nor 'val', nor 'hour', nor 'minute' parameters!\n", strCode, tostring(k)))
					end
				end
				...
				if type(monster_data.positions) ~= "table" then
					Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'positions' is not a table type!\n", strCode))
					break
				end
				...
				for k,v in ipairs(monster_data.positions) do
					...
					if type(v[1]) ~= "string" then
						bSucc = false
						Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'positions[%s][1]' is not a string type!\n", strCode, tostring(k)))
						break
					end
					...
					if not pMap then
						bSucc = false
						Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'positions[%s][1]' is not a valid map code!\n", strCode, tostring(k)))
						break
					end
					...
					if type(v[2]) ~= "number" or math.type(v[2]) ~= "integer" then
						bSucc = false
						Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'positions[%s][2]' is not an integer type!\n", strCode, tostring(k)))
						break
					end
					...
					if MapFld.m_nMapType ~= 0 and v[2] >= MapFld.m_nLayerNum then
						bSucc = false
						Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'positions[%s][2]' layer id (%d) >= Max layer (%d) !\n", strCode, tostring(k), v[2], MapFld.m_nLayerNum))
						break
					end

					if MapFld.m_nMapType == 0 and v[2] ~= 0 then
						bSucc = false
						Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'positions[%s][2]' layer id (%d) != 0 (%d) !\n", strCode, tostring(k), v[2], MapFld.m_nLayerNum))
						break
					end
					...
					for i = 3, 5 do
						if type(v[i]) ~= "number" then
							bSucc = false
							Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'positions[%s][%d]' is not a numeric type!\n", strCode, tostring(k), i))
							break
						end
					end
					...
					if not pMap:IsMapIn(v[3], v[4], v[5]) then
						bSucc = false
						Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'positions[%s]' not in Map range!\n", strCode, tostring(k)))
						break
					end

					if pMap.m_Level:GetNextYposForServer(v[3], v[4], v[5]) ~= 1 then
						bSucc = false
						Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'positions[%s]' there is no valid Y coordinate for this position!\n", strCode, tostring(k)))
						break
					end
					...
					if v[6] and (type(v[6]) ~= "number" or math.type(v[6]) ~= "integer") then
						bSucc = false
						Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'positions[%s][6]' is not an integer type!\n", strCode, tostring(k)))
						break
					end
				...
				if monster_data.window and type(monster_data.window) ~= "number" then
					Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'window' is not a numeric type!\n", strCode))
					break
				end
				...
				if monster_data.succRate and type(monster_data.succRate) ~= "number" then
					Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'succRate' is not a numeric type!\n", strCode))
					break
				end
				...
				if monster_data.duration and type(monster_data.duration) ~= "number" then
					Sirin.console.LogEx(ConsoleForeground.RED, ConsoleBackground.BLACK, string.format("Lua. sirinMonsterScheduleMgr.validateScriptData() id: %s 'duration' is not a numeric type!\n", strCode))
					break
				end

				newSchedule.duration = monster_data.duration and math.floor(monster_data.duration) or 0
				SirinTmp_MonsterSchedule[strCode] = newSchedule

			until true
		end

		if not bSucc then
			SirinTmp_MonsterSchedule = nil
			Sirin.WriteA(sirinMonsterScheduleMgr.m_pszLogPath, "Lua. sirinMonsterScheduleMgr.validateScriptData() bSucc == false!\n", true, true)
		end
...
function sirinMonsterScheduleMgr.onLoop()
	sirinMonsterScheduleMgr.m_tLoopTime = os.time()

	for k,v in pairs(sirinMonsterScheduleMgr.m_SpawnRules) do
		if v.m_Script:isActive() then
			if v.m_luaObj:isLive() and v.m_Script.duration > 0 and sirinMonsterScheduleMgr.m_tLoopTime > v.m_tLastBirthTime + v.m_Script.duration then
				v.m_luaObj:despawn()
				v.m_tLastDeathTime = sirinMonsterScheduleMgr.m_tLoopTime
				Sirin.WritePrivateProfileStringA(k, "LastDeath", tostring(v.m_tLastDeathTime), sirinMonsterScheduleMgr.m_pszSavePath)
				...
				if sirinMonsterScheduleMgr.m_bDebugLog then
					...
					print(os.date(_, sirinMonsterScheduleMgr.m_tLoopTime) .. " " .. emsg)
					Sirin.WriteA(sirinMonsterScheduleMgr.m_pszLogPath, emsg, true, true)
				end
			end

			if sirinMonsterScheduleMgr.m_tLoopTime > v.m_tNextScheduleRefreshTime and v.m_Script.schedule.scheduleType ~= "delay" then
				local l, n = getScheduleTime(v.m_Script)
				v.m_tLastScheduleRefreshTime = l
				v.m_tNextScheduleRefreshTime = n
			end
			...
			if not v.m_luaObj:isLive() then
				if v.m_tNextBirthTime > v.m_tLastBirthTime and sirinMonsterScheduleMgr.m_tLoopTime > v.m_tNextBirthTime and v.m_Script:isActive(v.m_tNextBirthTime) then
					v.m_tLastBirthTime = v.m_tNextBirthTime
					Sirin.WritePrivateProfileStringA(k, "LastBirth", tostring(v.m_tLastBirthTime), sirinMonsterScheduleMgr.m_pszSavePath)
			...
			if sirinMonsterScheduleMgr.m_tLoopTime > v.m_tNextBirthTime and not v:isInNextSummonWindow(v.m_tNextBirthTime) and v.m_Script.schedule.scheduleType ~= "delay" then
				v:updateSummonTime()
			end
		else
			...
			Sirin.WritePrivateProfileStringA(k, "NextBirth", "0", sirinMonsterScheduleMgr.m_pszSavePath)
			Sirin.WritePrivateProfileStringA(k, "LastBirth", "0", sirinMonsterScheduleMgr.m_pszSavePath)
			Sirin.WritePrivateProfileStringA(k, "LastDeath", "0", sirinMonsterScheduleMgr.m_pszSavePath)
...
function sirinMonsterScheduleMgr.saveState()
	...
	for _,v in pairs(sirinMonsterScheduleMgr.m_SpawnRules) do
...
function sirinMonsterScheduleMgr.init()
	...
	if SirinTmp_MonsterSchedule then
		---@type table<string, SirinMonsterScheduleScript>
		SirinScript_MonsterSchedule = SirinTmp_MonsterSchedule
		SirinTmp_MonsterSchedule = nil
	end

	if type(SirinScript_MonsterSchedule) ~= 'table' then
		return
	end

	sirinMonsterScheduleMgr.m_Monsteres = {}
	local oldRules = sirinMonsterScheduleMgr.m_SpawnRules
	sirinMonsterScheduleMgr.m_SpawnRules = {}
	sirinMonsterScheduleMgr.m_tLoopTime = os.time()

	for k, v in pairs(SirinScript_MonsterSchedule) do
		local newRule = SirinMonsterSpawnRule:new()
		sirinMonsterScheduleMgr.m_SpawnRules[k] = newRule
		newRule.m_Script = v
		...
		elseif t[k] then
			...
			local len, str = Sirin.GetPrivateProfileStringA(k, "NextBirth", "0", sirinMonsterScheduleMgr.m_pszSavePath)
			newRule.m_tNextBirthTime = tonumber(str) or 0
			len, str = Sirin.GetPrivateProfileStringA(k, "LastBirth", "0", sirinMonsterScheduleMgr.m_pszSavePath)
			newRule.m_tLastBirthTime = tonumber(str) or 0
			len, str = Sirin.GetPrivateProfileStringA(k, "LastDeath", "0", sirinMonsterScheduleMgr.m_pszSavePath)
		...
		if not newRule.m_Script:isActive() then
			...
			if not bShouldBeSpawned and newRule.m_tNextBirthTime > newRule.m_tLastBirthTime and sirinMonsterScheduleMgr.m_tLoopTime > newRule.m_tNextBirthTime and newRule.m_Script:isActive(newRule.m_tNextBirthTime) then
				bShouldBeSpawned = true
				newRule.m_tLastBirthTime = newRule.m_tNextBirthTime
				Sirin.WritePrivateProfileStringA(k, "LastBirth", tostring(newRule.m_tLastBirthTime), sirinMonsterScheduleMgr.m_pszSavePath)
			end
		end
		...
		if newRule.m_luaObj:isLive()then
			...
			elseif v.monster.m_dwIndex ~= newRule.m_luaObj.m_pCObj.m_pMonRec.m_dwIndex then
				...
				sirinMonsterScheduleMgr.m_Monsteres[newRule.m_luaObj.m_dwObjectSerial] = newRule.m_luaObj
			end
		end
...
return sirinMonsterScheduleMgr
